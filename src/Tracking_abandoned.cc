...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        //特征点
        const vector<cv::KeyPoint>  &kp1=mInitialFrame.mvKeysUn;
        const vector<cv::KeyPoint>  &kp2=mCurrentFrame.mvKeysUn;
        //描述子
        const cv::Mat &d1=mInitialFrame.mDescriptors;
        const cv::Mat &d2=mCurrentFrame.mDescriptors;    
        const cv::Mat img1=mInitialFrame.imgLeft;
        const cv::Mat img2=mCurrentFrame.imgLeft;
        ...
        int nmatches=matches_gms.size();
        cout<<"MonocularInitialization nmatches : "<<nmatches<<endl;
...
//     int nmatches = matcher.SearchByBoW(mpReferenceKF,mCurrentFrame,vpMapPointMatches);
    ...
    //特征点
    const vector<cv::KeyPoint>  &kp1=mpReferenceKF->mvKeysUn;
    const vector<cv::KeyPoint>  &kp2=mCurrentFrame.mvKeysUn;
    //描述子
    const cv::Mat &d1=mpReferenceKF->mDescriptors;
    const cv::Mat &d2=mCurrentFrame.mDescriptors;    
    const cv::Mat img1=mpReferenceKF->imgLeft;
    const cv::Mat img2=mCurrentFrame.imgLeft;
    vector<DMatch> matches_all, matches_gms;
    std::vector<bool> vbInliers;
    gms_matcher gms(kp1, img1.size(), kp2, img2.size(), matches_all);
